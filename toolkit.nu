...
#
# Why --fail is the default, with --no-fail to opt out: without it nutest exits
# 0 on a failing suite, so a run with 45 failures in it reads as success to
# anything that only looks at the exit code — including an agent.
...
# Pass --threads 0 for nutest's own default (0 means one per core).
