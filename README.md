...
- `<stem>.<ext>.<signer>.sig` (one per signature found beside the stamped file) — SSH signature over the snapshot, copied in at stamp time so it survives the next `seal` (which overwrites the live sig). `seal` always signs before it stamps, so its own bundles always carry one; `ots stamp` run on its own carries whatever signatures already sit beside the file (pinned by the test "seal stamps the root statement into the target repo and bundles its signature")
