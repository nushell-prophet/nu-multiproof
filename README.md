...
2. **Fetches the raw 80-byte block header** and recomputes everything locally — the double-SHA256 block hash, the merkle-root binding (the header must commit to the exact value the proof's operations replay to), and the proof-of-work: `bits` must decode to a target no easier than mainnet's `powLimit`, and the block hash must meet it. Read what that does and does not rule out: a cheaply forged header is out — a regtest-difficulty header with nonce 0 passed before the floor existed, because `bits` is read out of the very header under examination. A header actually mined at difficulty 1 is not — that costs ~2^32 hashes, minutes on a GPU, and the check has no notion of the difficulty expected at that height. So an explorer serving a fabricated header is bounded by that work, not by Bitcoin's. (Pinned by the test "check-block-header rejects a header below mainnet minimum difficulty".)
